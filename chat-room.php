            ...
            <form id="uploadForm" enctype="multipart/form-data">
                ...
                <div id="uploadContainer" class="hide">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" class="cancel" onclick="cancelUpload()">
                        <path
                            style="fill:#ff0000;stroke:#ffffff;stroke-width:3.8025;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:4;stroke-dasharray:none;stroke-opacity:1"
...
<script>
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    var identifiers = messages.map(function (message) {
                        return message.identifier;
                    });

                    identifiers.forEach(function (identifier, index) {
                        var isFile = messages[index].isFile;
                        isFileMap[identifier] = isFile;
                    });
                        ...
                        messages.forEach(function (message) {
                            if (!sentUnivocos.includes(message.univoco)) {
                                ...
                                if (!document.querySelector('[data-univoco="' + message.identifier + '"]')) {
                                    displayMessage(message.text, message.author, message.identifier, message.time, isFileMap);
    ...
    function displayMessage(message, author, identifier, time, isFileMap) {
        ...
        if (!dates.includes(date)) {
            ...
            if (!dateOfMessagesCreated[date]) {
                var dateOfMessages = `<p class="dateOfMessages">${date}</p>`;
                dayContainers[date].innerHTML += dateOfMessages;
        ...
        var isUserMessage = author === username;
        var isFile = isFileMap && isFileMap[identifier];
        var deleteIconHTML = isUserMessage ? `<i class="fas fa-trash red-icon" onclick="deleteMessage()"; data-univoco="${identifier}"></i>` : '';
        var downloadButtonHTML = isFile ? `<button class="downloadButton" data-univoco="${identifier}" onclick="downloadFile(this)">DOWNLOAD</button>` : '';

		var previousLastMessage = document.querySelector('.message-container.last-message');
    	if (previousLastMessage) {
        	previousLastMessage.classList.remove('last-message');
    	}

        var newMessageHTML = `
        <div class="message-container ${!loadAllMessages ? 'last-message' : ''}" style="${isUserMessage ? 'margin-right: 25px; justify-content: flex-end;' : ''}" data-univoco="${identifier}">
            <h5 style="color: white; ${isUserMessage ? 'margin: 15px 15px 0px 15px !important; display: inline-flex; margin-right: 10px;' : 'margin: 15px 0px 0px 10px;'}">
                ${author}
            </h5>
            <p style="margin: 10px 0; text-align: left; ${isFile ? 'padding: 23px 25px 45px 30px;' : ''}">${message}</p>
            <span style="float: right; ${isUserMessage ? 'right: 186px;' : 'right: 838px;'}">${formatTimeToHHMM(time)}</span>
            ${deleteIconHTML}
            ${downloadButtonHTML}
        </div>
    `;

        dayContainers[date].innerHTML += newMessageHTML;

        if (!dayContainers[date].parentNode) {
            messageContainer.appendChild(dayContainers[date]);
        }
        
        var lastMessageElement = document.querySelector('.message-container.last-message');
    	if (lastMessageElement) {
        	lastMessageElement.addEventListener('animationend', function() {
            	lastMessageElement.classList.remove('last-message');
        	});
    	}
    }
    ...
    function downloadFile(element) {
        var fileName = element.getAttribute("data-univoco");
        var xhr = new XMLHttpRequest();
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                var binaryString = atob(response.fileContent);
        ...
        xhr.open("POST", "downloadFile.php", true);
        xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        xhr.send("file=" + encodeURIComponent(fileName) + "&chatcode=" + encodeURIComponent("<?php echo $code; ?>"));
